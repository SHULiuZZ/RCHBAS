...
def p2(path):
    # video_path = "D:\\djangoproject\\media\\20230214-143601.mp4"  ###输入视频的路径
    ...
    def predict_long_audio():
        ...
        print(
            f"最终结果，消耗时间：{int(round((time.time() - start) * 1000))}, 得分: {int(sum(scores) / len(scores))}, 识别结果: {texts}")
    ...
    def predict_audio():
        ...
        vza.main(video_path)
        wav_emotion = []
        wav_files = glob.glob(os.path.join(r'D:\djangoproject\zjw\PPASR-master\newaudio', "*.wav"))
    ...
    def emotional(emotionaltext):
        ...
        model = AutoModelForSequenceClassification.from_pretrained('./bert_localpath/', num_labels=6)
        ...
        label = {0: '快乐', 1: '愤怒', 2: '悲伤', 3: '焦虑', 4: '疲惫', 5: '中性'}
        tokenizer = AutoTokenizer.from_pretrained('./bert_localpath/')
